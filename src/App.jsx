...
// FIX 1: Set API_URL to the base domain/port ONLY. 
// The fetch calls will then correctly add the /api/auth/ or /api/teacher/ prefix.
const API_URL = import.meta.env.PROD 
    ? 'https://smart-attendance-1-49fb.onrender.com' 
    : 'http://localhost:3001';
...
                // FIX 2: Prepend /api to the fetch path
                const lectureRes = await fetch(`${API_URL}/api/teacher/lectures/${userData.id}`, { headers: { 'Authorization': `Bearer ${userToken}` } });
...
                const studentsRes = await fetch(`${API_URL}/api/teacher/all-students`, { headers: { 'Authorization': `Bearer ${userToken}` } });
                const allAttendanceRes = await fetch(`${API_URL}/api/teacher/all-attendance`, { headers: { 'Authorization': `Bearer ${userToken}` } });
...
                    // FIX 2: Prepend /api to the fetch path
                    fetch(`${API_URL}/api/student/lectures`, { headers: { 'Authorization': `Bearer ${userToken}` } }),
                    fetch(`${API_URL}/api/student/attendance/${userData.id}`, { headers: { 'Authorization': `Bearer ${userToken}` } })
                ]);
                
                if (!lectureRes.ok) throw new Error('Failed to fetch lectures');
                lectureData = await lectureRes.json();
...
        try {
            // FIX 2: Correctly construct the full URL path from the base API_URL
            const res = await fetch(`${API_URL}/api/auth/${role}/login`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email, password }) });
...
        try {
            // FIX 2: Correctly construct the full URL path from the base API_URL
            const res = await fetch(`${API_URL}/api/teacher/lectures`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` }, body: JSON.stringify({ ...lectureData, teacher_id: user.id }) });
...
        try {
            // FIX 2: Correctly construct the full URL path from the base API_URL
            const res = await fetch(`${API_URL}/api/student/mark-attendance`, { 
                method: 'POST', 
